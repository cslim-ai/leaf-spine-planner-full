...
이 프로젝트는 정적 파일 기반입니다. 별도 빌드 없이 `index.html`을 브라우저에서 열 수 있습니다.

로컬 서버로 확인하려면 예를 들어 다음처럼 실행할 수 있습니다.
...
테스트는 Node.js로 직접 실행합니다. 현재 `package.json`은 없으므로 각 테스트 파일을 개별 실행합니다.

```powershell
node tests/calculator.test.js
node tests/diagram-geometry.test.js
node tests/diagram-highlight.test.js
node tests/input-state.test.js
node tests/message-copy.test.js
node tests/render-scheduler.test.js
node tests/report-layout.test.js
```
...
GitHub Pages에 배포하는 경우 저장소 Pages 설정에서 배포 브랜치와 루트 경로를 지정합니다.
